#!/usr/bin/env python
"""
HVG Count Sensitivity Analysis for Norman19.

Investigates how the number of highly variable genes selected affects the
technical duplicate positive control and calibration (DRF). Starts from raw
Norman19 data, fixes cell count at 256 per perturbation (128 per half), and
sweeps n_top_genes in [128, 256, 512, 1024, 2048, 4096, 8192, 16384].

Three-stage cached pipeline:
  Stage 1: Preprocessing + baselines -> cached h5ad per HVG count
  Stage 2: Metrics + DRF -> cached CSV
  Stage 3: Plotting -> PNG files

Usage:
    python analyses/calibration/hvg_sensitivity.py
    python analyses/calibration/hvg_sensitivity.py --force
    python analyses/calibration/hvg_sensitivity.py --plot-only
    python analyses/calibration/hvg_sensitivity.py --workers 4
"""
...
CACHE_DIR = PROJECT_ROOT / "analyses" / "calibration" / "hvg_cache"
RESULTS_DIR = PROJECT_ROOT / "analyses" / "calibration" / "results"

HVG_COUNTS = [128, 256, 512, 1024, 2048, 4096, 8192, 16384]
CELLS_PER_PERT = 256
HALF_CELLS = CELLS_PER_PERT // 2  # 128
MAX_CELLS_CONTROL = 8192
DATASET_NAME = "norman19"

METRICS_CSV = RESULTS_DIR / "hvg_sensitivity.csv"
SUMMARY_CSV = RESULTS_DIR / "hvg_sensitivity_summary.csv"
...
def load_and_prepare_raw_data():
    """Load raw Norman19 data, filter cells/genes, cap control, assign
    tech_dup_split, and remove perturbations with insufficient cells.
    ...
    # Assign one-time tech_dup_split
    ...
    # Remove perturbations with < CELLS_PER_PERT total cells
    print(f"Removing perturbations with < {CELLS_PER_PERT} total cells...")
    pert_counts = adata.obs[adata.obs['condition'] != 'control']['condition'].value_counts()
    eligible_perts = pert_counts[pert_counts >= CELLS_PER_PERT].index.tolist()
    ...
    for pert in eligible_perts:
        ...
        if fh.sum() < HALF_CELLS or sh.sum() < HALF_CELLS:
            print(f"  WARNING: {pert} has {fh.sum()}/{sh.sum()} first/second half "
                  f"(need >= {HALF_CELLS})")
    ...
    print(f"  Fixed cells per pert: {CELLS_PER_PERT} ({HALF_CELLS} per half)")
    print(f"  HVG counts to test: {HVG_COUNTS}")
    print()

    return adata


def process_single_hvg_count(args):
    """Process a single HVG count: subsample cells, normalize, HVG select,
    DEGs, baselines.

    Args:
        args: tuple of (adata_path_or_obj, n_hvg, cache_dir, force)

    Returns:
        Tuple of (n_hvg, output_path, success, error_msg)
    """
    adata_input, n_hvg, cache_dir, force = args

    output_path = Path(cache_dir) / f"norman19_hvg{n_hvg}.pkl"

    if output_path.exists() and not force:
        print(f"[hvg={n_hvg}] Cache exists at {output_path}, skipping")
        return (n_hvg, str(output_path), True, None)
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Step 1: Subsample cells (fixed at HALF_CELLS per half)
        # ------------------------------------------------------------------
        np.random.seed(42 + n_hvg)
        ...
        for condition in non_ctrl_conditions:
            ...
            if len(fh_cells) < HALF_CELLS or len(sh_cells) < HALF_CELLS:
                print(f"[hvg={n_hvg}] WARNING: {condition} has only "
                      f"{len(fh_cells)}/{len(sh_cells)} cells, need {HALF_CELLS}. Skipping.")
                continue

            sampled_fh = np.random.choice(fh_cells, size=HALF_CELLS, replace=False)
            sampled_sh = np.random.choice(sh_cells, size=HALF_CELLS, replace=False)
        ...
        print(f"[hvg={n_hvg}] Subsampled {n_perts_sampled} perturbations to "
              f"{CELLS_PER_PERT} cells each ({HALF_CELLS} per half). "
              f"Total cells: {adata.shape[0]}")
        ...
        sc.pp.log1p(adata)
        print(f"[hvg={n_hvg}] Normalized and log-transformed. Shape: {adata.shape}")

        # ------------------------------------------------------------------
        # Step 3: HVG selection (this is the variable!)
        # ------------------------------------------------------------------
        # Clamp n_hvg to available genes
        actual_n_hvg = min(n_hvg, adata.shape[1])
        sc.pp.highly_variable_genes(adata, n_top_genes=actual_n_hvg, subset=True)
        print(f"[hvg={n_hvg}] Selected {adata.shape[1]} HVGs (requested {n_hvg})")
        ...
        print(f"[hvg={n_hvg}] Computing second-half DEGs for {len(valid_sh_perts)} perturbations "
              f"(control excluded)...")
        ...
        print(f"[hvg={n_hvg}] Computed second-half DEGs for {len(names_df_dict)} perturbations")
        ...
        print(f"[hvg={n_hvg}] Computing first-half (GT) DEGs for {len(valid_fh_perts)} "
              f"perturbations (control excluded)...")
        ...
        print(f"[hvg={n_hvg}] Computed first-half (GT) DEGs for {len(names_df_dict_gt)} perturbations")
        ...
        print(f"[hvg={n_hvg}] Ground truth baseline: {len(gt_df)} perturbations")
        ...
        print(f"[hvg={n_hvg}] Technical duplicate baseline: {len(td_df)} perturbations")
        ...
        mean_baseline_df = pd.DataFrame(
            [dataset_mean.values], index=[DATASET_NAME], columns=var_names
        ).astype(float)
        adata.uns['dataset_mean_baseline'] = mean_baseline_df
        print(f"[hvg={n_hvg}] Mean baseline from {len(all_pert_means)} perturbation means")
        ...
        if len(ctrl_cells) > 0:
            ...
            ctrl_baseline_df = pd.DataFrame(
                [ctrl_mean], index=[DATASET_NAME], columns=var_names
            ).astype(float)
            adata.uns['ctrl_baseline'] = ctrl_baseline_df
            print(f"[hvg={n_hvg}] Control baseline from {len(ctrl_cells)} control cells")
        ...
        for condition_key in td_df.index:
            if condition_key not in pvals_adj_df_dict or condition_key not in names_df_dict:
                interp_df.loc[condition_key] = mean_values
                continue
            pvals_series = pd.Series(pvals_adj_df_dict[condition_key],
                                     index=names_df_dict[condition_key])
            pvals_ordered = pvals_series.reindex(var_names, fill_value=1.0)
            pvals = pvals_ordered.values.astype(float)
            alphas = np.nan_to_num(1 - pvals, nan=0.0)
            interpolated_values = (
        ...
        print(f"[hvg={n_hvg}] Interpolated baseline: {len(interp_df)} perturbations")

        print(f"[hvg={n_hvg}] Computed baselines: GT={len(gt_df)}, TD={len(td_df)}, "
              f"mean, ctrl, interp={len(interp_df)}")

        # ------------------------------------------------------------------
        # Step 6: Save as pickle (h5ad can't handle large DataFrames in uns)
        # ------------------------------------------------------------------
        Path(cache_dir).mkdir(parents=True, exist_ok=True)
        cache_data = {
            'var_names': var_names,
            'technical_duplicate_first_half_baseline': gt_df,
            'technical_duplicate_second_half_baseline': td_df,
            'dataset_mean_baseline': mean_baseline_df,
            'ctrl_baseline': ctrl_baseline_df,
            'interpolated_duplicate_baseline': interp_df,
            'names_df_dict': names_df_dict,
            'pvals_adj_df_dict': pvals_adj_df_dict,
            'scores_df_dict': scores_df_dict,
            'names_df_dict_gt': names_df_dict_gt,
            'pvals_adj_df_dict_gt': pvals_adj_df_dict_gt,
            'scores_df_dict_gt': scores_df_dict_gt,
        }
        with open(output_path, 'wb') as f:
            pickle.dump(cache_data, f)
        elapsed = time.time() - t_start
        print(f"[hvg={n_hvg}] Saved to {output_path} ({elapsed:.1f}s)")
        ...
        import traceback
        traceback.print_exc()
        return (n_hvg, str(output_path), False, str(e))


def run_stage1(force=False, workers=1):
    """Run Stage 1: Preprocessing + Baselines for all HVG counts."""
    print("\n" + "=" * 60)
    print("STAGE 1: PREPROCESSING + BASELINES")
    print("=" * 60)

    to_process = []
    for n_hvg in HVG_COUNTS:
        output_path = CACHE_DIR / f"norman19_hvg{n_hvg}.pkl"
        if output_path.exists() and not force:
            print(f"  [hvg={n_hvg}] Cache exists, skipping")
        else:
            to_process.append(n_hvg)

    if not to_process:
        print("All HVG counts already cached. Use --force to recompute.")
        return

    print(f"Processing HVG counts: {to_process}")

    adata = load_and_prepare_raw_data()

    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    temp_path = CACHE_DIR / "_prepared_raw.h5ad"
    print(f"Saving prepared data to temp file: {temp_path}")
    adata.write_h5ad(str(temp_path))

    args_list = [(str(temp_path), n_hvg, str(CACHE_DIR), force) for n_hvg in to_process]

    if workers > 1 and len(to_process) > 1:
        print(f"\nRunning {len(to_process)} HVG counts in parallel with {workers} workers...")
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(process_single_hvg_count, a): a[1] for a in args_list}
            for future in as_completed(futures):
                n_hvg = futures[future]
                result = future.result()
                if not result[2]:
                    print(f"  [hvg={n_hvg}] FAILED: {result[3]}")
    else:
        print(f"\nRunning {len(to_process)} HVG counts sequentially...")
        for a in args_list:
            result = process_single_hvg_count(a)
            if not result[2]:
                print(f"  [hvg={a[1]}] FAILED: {result[3]}")
...
def compute_nir(predictions_df, ground_truth_df):
    ...
    for i, pert_key in enumerate(common_keys):
        ...
        comparisons = [1 if correct_distance < distance_matrix[i, j] else 0
                       for j in range(len(common_keys)) if i != j]
        nir_scores[pert_key] = np.mean(comparisons) if comparisons else 0.0
    return nir_scores


def compute_metrics_for_hvg_count(n_hvg, cache_dir):
    pkl_path = Path(cache_dir) / f"norman19_hvg{n_hvg}.pkl"
    if not pkl_path.exists():
        print(f"[hvg={n_hvg}] Cache not found at {pkl_path}, skipping")
        return []

    print(f"[hvg={n_hvg}] Loading {pkl_path}...")
    with open(pkl_path, 'rb') as f:
        cache_data = pickle.load(f)

    var_names = cache_data['var_names']

    gt_df = cache_data['technical_duplicate_first_half_baseline']
    td_df = cache_data['technical_duplicate_second_half_baseline']
    mean_df = cache_data['dataset_mean_baseline']
    ctrl_df = cache_data['ctrl_baseline']
    interp_df = cache_data['interpolated_duplicate_baseline']

    mean_vec = mean_df.loc[DATASET_NAME].values.astype(float)
    ctrl_vec = ctrl_df.loc[DATASET_NAME].values.astype(float)

    scores_dict_gt = cache_data['scores_df_dict_gt']
    names_dict_gt = cache_data['names_df_dict_gt']
    pvals_dict_gt = cache_data['pvals_adj_df_dict_gt']

    print(f"[hvg={n_hvg}] Computing DEG weights and masks...")
    ...
    print(f"[hvg={n_hvg}] DEG weights: {len(weights_dict)}, DEG masks: {len(masks_dict)}")
    ...
    common_perts = [p for p in common_perts
                    if 'control' not in p.lower() and 'ctrl' not in p.lower()]
    print(f"[hvg={n_hvg}] Computing metrics for {len(common_perts)} perturbations")

    baselines_dict = {
        'dataset_mean': pd.DataFrame([mean_vec] * len(common_perts),
                                      index=common_perts, columns=var_names),
        'technical_duplicate': td_df.loc[common_perts],
        'control': pd.DataFrame([ctrl_vec] * len(common_perts),
                                 index=common_perts, columns=var_names),
        'interpolated_duplicate': interp_df.loc[common_perts],
    }
    gt_common = gt_df.loc[common_perts]

    print(f"[hvg={n_hvg}] Computing NIR scores...")
    ...
    for baseline_name, baseline_df in baselines_dict.items():
        ...
        for pert_key in common_perts:
            ...
            results.append({'n_hvg': n_hvg, 'baseline': baseline_name,
            ...
            if deg_mask.sum() > 2:
                try:
                    pdc_deg_val, _ = pearsonr(pred_delta_ctrl[deg_mask], gt_delta_ctrl[deg_mask])
                except Exception:
                    pdc_deg_val = np.nan
            else:
                pdc_deg_val = np.nan
        ...
        print(f"[hvg={n_hvg}] {baseline_name}: "
...
def run_stage2(force=False, workers=1):
    ...
    if METRICS_CSV.exists() and not force:
        print(f"Metrics CSV exists at {METRICS_CSV}. Use --force to recompute.")
        return

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    all_results = []

    if workers > 1 and len(HVG_COUNTS) > 1:
        print(f"Computing metrics for {len(HVG_COUNTS)} HVG counts with {workers} workers...")
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(compute_metrics_for_hvg_count, n, str(CACHE_DIR)): n
                       for n in HVG_COUNTS}
            for future in as_completed(futures):
                n_hvg = futures[future]
                try:
                    ...
                    print(f"[hvg={n_hvg}] FAILED: {e}")
    else:
        print(f"Computing metrics for {len(HVG_COUNTS)} HVG counts sequentially...")
        for n_hvg in HVG_COUNTS:
            all_results.extend(compute_metrics_for_hvg_count(n_hvg, str(CACHE_DIR)))
    ...
    print(f"\nSaved metrics to {METRICS_CSV} ({len(results_df)} rows)")

    summary = results_df.groupby(['n_hvg', 'baseline', 'metric'])['value'].agg(
        ['mean', 'median', 'std', 'count']).reset_index()
    summary.columns = ['n_hvg', 'baseline', 'metric', 'mean', 'median', 'std', 'count']
    summary['sem'] = summary['std'] / np.sqrt(summary['count'])
    summary.to_csv(SUMMARY_CSV, index=False)
    print(f"Saved summary to {SUMMARY_CSV} ({len(summary)} rows)")
...
def run_stage3():
    ...
    if not SUMMARY_CSV.exists() or not METRICS_CSV.exists():
        print("CSVs not found. Run stages 1+2 first.")
        return
    ...
    baseline_colors = {
        'dataset_mean': '#1f77b4', 'technical_duplicate': '#2ca02c',
        'control': '#d62728', 'interpolated_duplicate': '#9467bd',
    }
    baseline_labels = {
        'dataset_mean': 'Dataset Mean', 'technical_duplicate': 'Technical Duplicate',
        'control': 'Control', 'interpolated_duplicate': 'Interpolated Duplicate',
    }
    baseline_order = ['dataset_mean', 'technical_duplicate', 'control', 'interpolated_duplicate']
    ...
    for idx, (metric_name, config) in enumerate(metrics_grid):
        ...
        for bl in baseline_order:
            bl_data = metric_data[metric_data['baseline'] == bl].sort_values('n_hvg')
            if len(bl_data) == 0:
                continue
            ax.errorbar(bl_data['n_hvg'], bl_data['mean'], yerr=bl_data['sem'],
                        label=baseline_labels[bl], color=baseline_colors[bl],
        ...
        ax.legend()
        ax.set_xscale('log')
        ax.set_xticks(HVG_COUNTS)
        ax.set_xticklabels([str(n) for n in HVG_COUNTS], rotation=45)
        ax.grid(True, alpha=0.3)

    plt.suptitle('HVG Count Sensitivity Analysis - Norman19', y=1.02)
    plt.tight_layout()
    metrics_plot_path = RESULTS_DIR / "hvg_sensitivity_metrics.png"
    ...
    sorted_hvgs = sorted(HVG_COUNTS)
    pos_map = {n: i for i, n in enumerate(sorted_hvgs)}

    # First pass: compute DRF
    all_drf_dfs = {}
    global_drf_min = 0.0
    for metric_name, config in metrics_grid:
        lower_is_better = config['lower_is_better']
        drf_data = []
        for n_hvg in sorted_hvgs:
            mask_n = results_df['n_hvg'] == n_hvg
    ...
    for metric_name, config in metrics_grid:
        ...
        print(f"\n  {config['title']}:")
        for n_hvg in sorted_hvgs:
            pts = drf_df[drf_df['n_hvg'] == n_hvg]['drf'].dropna()
            if len(pts) == 0:
                continue
            print(f"    hvg={n_hvg:>5d}: median={pts.median():.3f}, "
    ...
    for idx, (metric_name, config) in enumerate(metrics_grid):
        ...
        for n_hvg in sorted_hvgs:
            pts = drf_df_clipped[drf_df_clipped['n_hvg'] == n_hvg]['drf_clipped'].values
            if len(pts) >= 2:
                violin_data.append(pts)
                violin_positions.append(pos_map[n_hvg])
        ...
        for n_hvg in sorted_hvgs:
            subset = drf_df_clipped[drf_df_clipped['n_hvg'] == n_hvg]
            if len(subset) == 0:
                continue
            x_jittered = pos_map[n_hvg] + rng.uniform(-0.2, 0.2, len(subset))
            normal = ~subset['is_clipped'].values
            if normal.any():
                ax.scatter(x_jittered[normal], subset['drf_clipped'].values[normal],
                           color='#2ca02c', alpha=0.3, s=10,
            ...
            if clipped.any():
                ax.scatter(x_jittered[clipped], subset['drf_clipped'].values[clipped],
                           color='#2ca02c', alpha=0.5, s=30,
                           marker='v', edgecolors='none', rasterized=True, zorder=4)

        medians = drf_df.groupby('n_hvg')['drf'].median().sort_index()
        ...
        ax.scatter(median_positions, median_vals,
                   color='white', edgecolors='#1b7a1b', s=50, zorder=6,
        ...
        ax.set_xticks(range(len(sorted_hvgs)))
        ax.set_xticklabels([str(n) for n in sorted_hvgs], rotation=45)
        ax.grid(True, alpha=0.3, axis='y')

    plt.suptitle('DRF vs HVG Count (Norman19)', y=1.02)
    plt.tight_layout()
    drf_plot_path = RESULTS_DIR / "hvg_sensitivity_drf.png"
...
def main():
    parser = argparse.ArgumentParser(description="HVG Count Sensitivity Analysis for Norman19")
    parser.add_argument('--force', action='store_true', help='Force recompute all stages')
    parser.add_argument('--plot-only', action='store_true', help='Only re-run plotting')
    ...
    print(f"\nTotal elapsed time: {time.time() - t_total:.1f}s")
