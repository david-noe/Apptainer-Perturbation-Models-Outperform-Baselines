#!/usr/bin/env python
"""
UMI Depth Sensitivity Analysis for Norman19.

Investigates how sequencing depth (UMI counts per cell) affects the technical
duplicate positive control and calibration (DRF). Starts from raw Norman19 data,
filters to cells with >= 8192 UMI counts, fixes cell count at 256 per
perturbation (128 per half), then for each target UMI depth
(256, 512, 1024, 2048, 4096, 8192) downsamples UMI counts via
sc.pp.downsample_counts, re-preprocesses from scratch (including DEGs),
recomputes all baselines, evaluates metrics, and shows how calibration
degrades with lower sequencing depth.

Three-stage cached pipeline:
  Stage 1: Preprocessing + baselines -> cached h5ad per UMI depth
  Stage 2: Metrics + DRF -> cached CSV
  Stage 3: Plotting -> PNG files

Usage:
    python analyses/calibration/umi_depth_sensitivity.py
    python analyses/calibration/umi_depth_sensitivity.py --force
    python analyses/calibration/umi_depth_sensitivity.py --plot-only
    python analyses/calibration/umi_depth_sensitivity.py --workers 4
"""
...
CACHE_DIR = PROJECT_ROOT / "analyses" / "calibration" / "umi_depth_cache"
RESULTS_DIR = PROJECT_ROOT / "analyses" / "calibration" / "results"

UMI_DEPTHS = [64, 128, 256, 512, 1024, 2048, 4096, 8192]
CELLS_PER_PERT = 256          # Fixed cell count per perturbation
HALF_CELLS = CELLS_PER_PERT // 2  # 128 per half
MIN_UMI_PER_CELL = np.max(UMI_DEPTHS)      # Only keep cells with >= this many total UMI counts
MAX_CELLS_CONTROL = 8192
DATASET_NAME = "norman19"
N_TOP_GENES = 8192

METRICS_CSV = RESULTS_DIR / "umi_depth_sensitivity.csv"
SUMMARY_CSV = RESULTS_DIR / "umi_depth_sensitivity_summary.csv"
...
def load_and_prepare_raw_data():
    """Load raw Norman19 data, filter cells/genes, filter by UMI count,
    cap control, assign tech_dup_split, and remove perturbations with
    insufficient cells.
    ...
    # 7. Filter cells by total UMI counts >= MIN_UMI_PER_CELL
    print(f"Filtering cells with total UMI counts >= {MIN_UMI_PER_CELL}...")
    umi_counts = np.array(adata.X.sum(axis=1)).flatten()
    adata.obs['total_umi'] = umi_counts
    n_before_umi = adata.shape[0]
    adata = adata[adata.obs['total_umi'] >= MIN_UMI_PER_CELL].copy()
    print(f"  Before: {n_before_umi} cells")
    print(f"  After:  {adata.shape[0]} cells")
    print(f"  UMI count range: {adata.obs['total_umi'].min():.0f} - {adata.obs['total_umi'].max():.0f}")

    # 8. Cap control cells at 8192
    np.random.seed(42)
    ...
    # 9. Assign one-time tech_dup_split (non-control only)
    ...
    # 10. Remove perturbations with < CELLS_PER_PERT total cells (keep control)
    ...
    # 11. Verify each eligible pert has enough cells in each half
    for pert in eligible_perts:
        ...
        if n_fh < HALF_CELLS or n_sh < HALF_CELLS:
            print(f"  WARNING: {pert} has {n_fh} first_half, {n_sh} second_half "
                  f"(need >= {HALF_CELLS} per half)")
    ...
    print(f"  UMI depths to test: {UMI_DEPTHS}")
    print()

    return adata


def process_single_umi_depth(args):
    """Process a single UMI depth: subsample cells, downsample UMIs,
    normalize, HVG, DEGs, baselines.

    This function is designed to be called in parallel via ProcessPoolExecutor.

    Args:
        args: tuple of (adata_path_or_obj, umi_depth, cache_dir, force)

    Returns:
        Tuple of (umi_depth, output_path, success, error_msg)
    """
    adata_input, umi_depth, cache_dir, force = args

    output_path = Path(cache_dir) / f"norman19_umi{umi_depth}.h5ad"

    # Check cache
    if output_path.exists() and not force:
        print(f"[umi={umi_depth}] Cache exists at {output_path}, skipping")
        return (umi_depth, str(output_path), True, None)
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Step 1: Subsample CELLS_PER_PERT/2 from each half (fixed cell count)
        # ------------------------------------------------------------------
        np.random.seed(42 + umi_depth)
        ...
        for condition in non_ctrl_conditions:
            ...
            if len(fh_cells) < HALF_CELLS or len(sh_cells) < HALF_CELLS:
                print(f"[umi={umi_depth}] WARNING: {condition} has only "
                      f"{len(fh_cells)}/{len(sh_cells)} cells in first/second half, "
                      f"need {HALF_CELLS}. Skipping.")
                continue
        ...
        print(f"[umi={umi_depth}] Subsampled {n_perts_sampled} perturbations to "
              f"{CELLS_PER_PERT} cells each ({HALF_CELLS} per half). "
              f"Total cells: {adata.shape[0]}")

        # ------------------------------------------------------------------
        # Step 2: Downsample UMI counts (on raw counts, BEFORE normalization)
        # ------------------------------------------------------------------
        print(f"[umi={umi_depth}] Downsampling UMI counts to {umi_depth} per cell...")
        sc.pp.downsample_counts(adata, counts_per_cell=umi_depth)
        umi_after = np.array(adata.X.sum(axis=1)).flatten()
        print(f"[umi={umi_depth}] UMI counts after downsampling: "
              f"mean={umi_after.mean():.0f}, min={umi_after.min():.0f}, "
              f"max={umi_after.max():.0f}")
        ...
        sc.pp.log1p(adata)
        print(f"[umi={umi_depth}] Normalized and log-transformed. Shape: {adata.shape}")
        ...
        print(f"[umi={umi_depth}] Selected {len(genes_to_keep)} genes "
              f"({n_hvg} HVGs + {n_pert_genes} perturbation genes)")

        # ------------------------------------------------------------------
        # Step 5: Compute DEGs (both halves, excluding control)
        # ------------------------------------------------------------------
        non_ctrl_conditions = [c for c in adata.obs['condition'].unique()
                               if 'control' not in c.lower() and 'ctrl' not in c.lower()]

        # 5a) Second-half DEGs (for interpolation p-values)
        ...
        print(f"[umi={umi_depth}] Computing second-half DEGs for {len(valid_sh_perts)} "
        ...
        print(f"[umi={umi_depth}] Computed second-half DEGs for {len(names_df_dict)} perturbations")

        # 5b) First-half DEGs (for GT metric weights)
        ...
        print(f"[umi={umi_depth}] Computing first-half (GT) DEGs for {len(valid_fh_perts)} "
              f"perturbations (control excluded)...")
        ...
        print(f"[umi={umi_depth}] Computed first-half (GT) DEGs for "
              f"{len(names_df_dict_gt)} perturbations")
        ...
        print(f"[umi={umi_depth}] Ground truth baseline: {len(gt_df)} perturbations")
        ...
        print(f"[umi={umi_depth}] Technical duplicate baseline: {len(td_df)} perturbations")
        ...
        print(f"[umi={umi_depth}] Mean baseline computed from "
              f"{len(all_pert_means)} perturbation means")
        ...
        if len(ctrl_cells) > 0:
            ...
            print(f"[umi={umi_depth}] Control baseline from {len(ctrl_cells)} control cells")
        ...
        print(f"[umi={umi_depth}] Interpolated baseline: {len(interp_df)} perturbations")

        print(f"[umi={umi_depth}] Computed baselines: GT={len(gt_df)}, TD={len(td_df)}, "
        ...
        print(f"[umi={umi_depth}] Saved to {output_path} ({elapsed:.1f}s)")
        ...
        import traceback
        traceback.print_exc()
        return (umi_depth, str(output_path), False, str(e))


def run_stage1(force=False, workers=1):
    """Run Stage 1: Preprocessing + Baselines for all UMI depths."""
    print("\n" + "=" * 60)
    print("STAGE 1: PREPROCESSING + BASELINES")
    print("=" * 60)

    # Check which UMI depths need processing
    to_process = []
    for umi_depth in UMI_DEPTHS:
        output_path = CACHE_DIR / f"norman19_umi{umi_depth}.h5ad"
        if output_path.exists() and not force:
            print(f"  [umi={umi_depth}] Cache exists, skipping")
        else:
            to_process.append(umi_depth)

    if not to_process:
        print("All UMI depths already cached. Use --force to recompute.")
        return

    print(f"Processing UMI depths: {to_process}")
    ...
    # Build args
    args_list = [(str(temp_path), umi_depth, str(CACHE_DIR), force)
                 for umi_depth in to_process]

    if workers > 1 and len(to_process) > 1:
        print(f"\nRunning {len(to_process)} UMI depths in parallel with {workers} workers...")
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(process_single_umi_depth, a): a[1]
                       for a in args_list}
            for future in as_completed(futures):
                umi_depth = futures[future]
                result = future.result()
                if not result[2]:
                    print(f"  [umi={umi_depth}] FAILED: {result[3]}")
    else:
        print(f"\nRunning {len(to_process)} UMI depths sequentially...")
        for a in args_list:
            result = process_single_umi_depth(a)
            if not result[2]:
                print(f"  [umi={a[1]}] FAILED: {result[3]}")
...
def compute_metrics_for_umi_depth(umi_depth, cache_dir):
    """Compute all metrics for a single UMI depth.

    Args:
        umi_depth: UMI counts per cell
        cache_dir: Path to cache directory

    Returns:
        List of dicts with columns: umi_depth, baseline, metric, perturbation, value
    """
    h5ad_path = Path(cache_dir) / f"norman19_umi{umi_depth}.h5ad"
    if not h5ad_path.exists():
        print(f"[umi={umi_depth}] h5ad not found at {h5ad_path}, skipping")
        return []

    print(f"[umi={umi_depth}] Loading {h5ad_path}...")
    ...
    print(f"[umi={umi_depth}] Computing DEG weights and masks...")
    ...
    print(f"[umi={umi_depth}] DEG weights for {len(weights_dict)} perturbations, "
          f"DEG masks for {len(masks_dict)} perturbations")

    # Common perturbation keys
    common_perts = sorted(
        set(gt_df.index) & set(td_df.index) & set(interp_df.index)
    )
    common_perts = [p for p in common_perts
                    if 'control' not in p.lower() and 'ctrl' not in p.lower()]
    print(f"[umi={umi_depth}] Computing metrics for {len(common_perts)} perturbations")
    ...
    print(f"[umi={umi_depth}] Computing NIR scores...")
    ...
    for baseline_name, baseline_df in baselines_dict.items():
        ...
        for pert_key in common_perts:
            ...
            results.append({'umi_depth': umi_depth, 'baseline': baseline_name,
        ...
        print(f"[umi={umi_depth}] {baseline_name}: "
...
def run_stage2(force=False, workers=1):
    ...
    if workers > 1 and len(UMI_DEPTHS) > 1:
        print(f"Computing metrics for {len(UMI_DEPTHS)} UMI depths with {workers} workers...")
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(compute_metrics_for_umi_depth, ud, str(CACHE_DIR)): ud
                for ud in UMI_DEPTHS
            }
            for future in as_completed(futures):
                ud = futures[future]
                try:
                    ...
                    print(f"[umi={ud}] FAILED: {e}")
    else:
        print(f"Computing metrics for {len(UMI_DEPTHS)} UMI depths sequentially...")
        for umi_depth in UMI_DEPTHS:
            result = compute_metrics_for_umi_depth(umi_depth, str(CACHE_DIR))
            all_results.extend(result)
    ...
    summary = results_df.groupby(['umi_depth', 'baseline', 'metric'])['value'].agg(
        ['mean', 'median', 'std', 'count']
    ).reset_index()
    summary.columns = ['umi_depth', 'baseline', 'metric', 'mean', 'median', 'std', 'count']
...
def run_stage3():
    ...
    for idx, (metric_name, config) in enumerate(metrics_grid):
        ...
        for baseline_name in baseline_order:
            bl_data = metric_data[metric_data['baseline'] == baseline_name].sort_values('umi_depth')
            if len(bl_data) == 0:
                continue

            ax.errorbar(
                bl_data['umi_depth'], bl_data['mean'], yerr=bl_data['sem'],
        ...
        ax.legend()
        ax.set_xscale('log')
        ax.set_xticks(UMI_DEPTHS)
        ax.set_xticklabels([str(n) for n in UMI_DEPTHS])
        ax.grid(True, alpha=0.3)

    plt.suptitle('UMI Depth Sensitivity Analysis - Norman19', y=1.02)
    plt.tight_layout()

    metrics_plot_path = RESULTS_DIR / "umi_depth_sensitivity_metrics.png"
    ...
    sorted_depths = sorted(UMI_DEPTHS)
    pos_map = {n: i for i, n in enumerate(sorted_depths)}
    ...
    for idx, (metric_name, config) in enumerate(metrics_grid):
        ...
        drf_data = []
        for umi_depth in sorted_depths:
            mask_n = results_df['umi_depth'] == umi_depth
            ...
            for _, row in merged.iterrows():
                ...
                drf_data.append({'umi_depth': umi_depth, 'drf': drf})
        if drf_data:
    ...
    for metric_name, config in metrics_grid:
        ...
        print(f"\n  {config['title']}:")
        for ud in sorted_depths:
            pts = drf_df[drf_df['umi_depth'] == ud]['drf'].dropna()
            if len(pts) == 0:
                continue
            print(f"    umi={ud:>5d}: median={pts.median():.3f}, "
    ...
    for idx, (metric_name, config) in enumerate(metrics_grid):
        ...
        # Build violin data (using clipped values)
        violin_data = []
        violin_positions = []
        for ud in sorted_depths:
            pts = drf_df_clipped[drf_df_clipped['umi_depth'] == ud]['drf_clipped'].values
            if len(pts) >= 2:
                violin_data.append(pts)
                violin_positions.append(pos_map[ud])
        ...
        for ud in sorted_depths:
            subset = drf_df_clipped[drf_df_clipped['umi_depth'] == ud]
            if len(subset) == 0:
                continue
            x_jittered = pos_map[ud] + rng.uniform(-0.2, 0.2, len(subset))

            # Normal points (not clipped)
            normal = ~subset['is_clipped'].values
            if normal.any():
                ax.scatter(x_jittered[normal], subset['drf_clipped'].values[normal],
                           color='#1f77b4', alpha=0.3, s=10,
            ...
            if clipped.any():
                ax.scatter(x_jittered[clipped], subset['drf_clipped'].values[clipped],
                           color='#1f77b4', alpha=0.5, s=30,
                           marker='v', edgecolors='none', rasterized=True, zorder=4)

        medians = drf_df.groupby('umi_depth')['drf'].median().sort_index()
        ...
        ax.scatter(median_positions, median_vals,
                   color='white', edgecolors='#1565c0', s=50, zorder=6,
        ...
        ax.set_xticklabels([str(n) for n in sorted_depths])
        ax.grid(True, alpha=0.3, axis='y')

    plt.suptitle('DRF vs UMI Depth (Norman19)', y=1.02)
    plt.tight_layout()

    drf_plot_path = RESULTS_DIR / "umi_depth_sensitivity_drf.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="UMI Depth Sensitivity Analysis for Norman19"
    )
