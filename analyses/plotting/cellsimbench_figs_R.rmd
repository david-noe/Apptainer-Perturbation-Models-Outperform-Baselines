...
# Classify metrics as "Main" or "DEG" based on whether they contain "_degs" suffix
metrics$metric_type <- ifelse(grepl("_degs$", metrics$metric), "DEG", "Main")
...
## Define metric labels (used by all heatmaps)
```{r}
metric_labels <- c(
  ...
  "pds"                    = "PDS",
  "mse"                    = "MSE",
  "mse_deltactrl"          = "MSE \u0394 Ctrl",
  "mse_degs"               = "MSE DEGs",
  "mse_deltactrl_degs"     = "MSE \u0394 Ctrl DEGs",
  "wmse"                   = "WMSE",
  "wmse_deltactrl"         = "WMSE \u0394 Ctrl",
  "mae"                    = "MAE",
  "mae_deltactrl"          = "MAE \u0394 Ctrl",
  "mae_degs"               = "MAE DEGs",
  "mae_deltactrl_degs"     = "MAE \u0394 Ctrl DEGs",
  "wmae"                   = "Weighted MAE",
  "wmae_deltactrl"         = "Weighted MAE \u0394 Ctrl",
...
## Helper function for generating DRF heatmaps
```{r}
# Reusable function to create DRF heatmap
create_drf_heatmap <- function(data, drf_col, metric_type_filter, title_suffix, metric_labels) {
  # Filter by metric type
  filtered_data <- data %>%
    filter(metric_type == metric_type_filter)
  
  # Calculate mean DRF per dataset and metric
  heatmap_long <- filtered_data %>%
    group_by(dataset, metric) %>%
    summarise(mean_drf = mean(.data[[drf_col]], na.rm = TRUE), .groups = "drop")
...
## DRF Heatmap - Technical Duplicate Baseline, Main Metrics
```{r fig.width=7, fig.height=5}
heatmap_techdup_main <- create_drf_heatmap(
  data = metrics,
  drf_col = "drf",
  metric_type_filter = "Main",
  title_suffix = "Technical Duplicate Baseline, Main Metrics",
  metric_labels = metric_labels
)
heatmap_techdup_main
ggsave("figures/fS8_main.png", heatmap_techdup_main, width = 7, height = 5, dpi = 300)
```

## DRF Heatmap - Technical Duplicate Baseline, DEG Metrics
```{r fig.width=7, fig.height=5}
heatmap_techdup_degs <- create_drf_heatmap(
  data = metrics,
  drf_col = "drf",
  metric_type_filter = "DEG",
  title_suffix = "Technical Duplicate Baseline, DEG Metrics",
  metric_labels = metric_labels
)
heatmap_techdup_degs
ggsave("figures/fS8_degs.png", heatmap_techdup_degs, width = 7, height = 5, dpi = 300)
```

# Figure 2E: DRF Heatmap Across Datasets and Metrics (Interpolated Baseline) 

## DRF Heatmap - Interpolated Baseline, Main Metrics
```{r fig.width=7, fig.height=5}
heatmap_interp_main <- create_drf_heatmap(
  data = metrics,
  drf_col = "drf_interpolated",
  metric_type_filter = "Main",
  title_suffix = "Interpolated Baseline, Main Metrics",
  metric_labels = metric_labels
)
heatmap_interp_main
ggsave("figures/f2e_main.png", heatmap_interp_main, width = 7, height = 5, dpi = 300)
```

## DRF Heatmap - Interpolated Baseline, DEG Metrics
```{r fig.width=7, fig.height=5}
heatmap_interp_degs <- create_drf_heatmap(
  data = metrics,
  drf_col = "drf_interpolated",
  metric_type_filter = "DEG",
  title_suffix = "Interpolated Baseline, DEG Metrics",
  metric_labels = metric_labels
)
heatmap_interp_degs
ggsave("figures/f2e_degs.png", heatmap_interp_degs, width = 7, height = 5, dpi = 300)
```

## DRF Heatmap - Control Baseline, Main Metrics
```{r fig.width=7, fig.height=5}
heatmap_ctrl_main <- create_drf_heatmap(
  data = metrics,
  drf_col = "drf_ctrl",
  metric_type_filter = "Main",
  title_suffix = "Control Baseline, Main Metrics",
  metric_labels = metric_labels
)
heatmap_ctrl_main
ggsave("figures/fS_ctrl_main.png", heatmap_ctrl_main, width = 7, height = 5, dpi = 300)
```

## DRF Heatmap - Control Baseline, DEG Metrics
```{r fig.width=7, fig.height=5}
heatmap_ctrl_degs <- create_drf_heatmap(
  data = metrics,
  drf_col = "drf_ctrl",
  metric_type_filter = "DEG",
  title_suffix = "Control Baseline, DEG Metrics",
  metric_labels = metric_labels
)
heatmap_ctrl_degs
ggsave("figures/fS_ctrl_degs.png", heatmap_ctrl_degs, width = 7, height = 5, dpi = 300)
```
...
if (!file.exists("gsea_input_for_R/fgsea_all_results.csv") | !file.exists("gsea_input_for_R/all_fgsea_results.rds")) {

  # Load ChEA gene sets
  chea_lines <- readLines("ENCODE_and_ChEA_Consensus_TFs_from_ChIP-X.txt")
...
test_m_wide <- test_m %>%
  select(-c("drf", "drf_mean", "drf_ctrl")) %>% 
  pivot_wider(names_from = "metric", values_from = "drf_interpolated") %>% 
